...
dental.DentalChart = class DentalChart {
    ...
    createToothElement(number, jaw) {
        const status = (this.frm.doc.dental_conditions || []).find(t => t.tooth == number);
    ...
    getToothStatus(toothNumber) {
        if (!this.frm) return '';
        const toothRecord = (this.frm.doc.dental_conditions || []).find(t => t.tooth == toothNumber);
    ...
    getToothImages(status, jaw, toothNumber) {
        ...
        return [
            ...
            `implant-${this.frm.doc.is_pediatric ? 'deciduous' : 'permanent'}-${jaw}-${numberToReturn}-incisal.png`,
    ...
    showPeriodontalSites(toothNumber) {
        ...
        sites.forEach(site => {
            ...
            const record = (this.frm.doc.periodontal_records || []).find(p =>
    ...
    bindPeriodontalEvents(toothNumber) {
        const saveSiteData = (field, site, value) => {
            ...
            let record = this.frm.doc.periodontal_records.find(p =>
                p.tooth == toothNumber && p.site == site
            );

            if (!record) {
                record = this.frm.add_child('periodontal_records', {
                    tooth: toothNumber,
                    site: site
                });
            }
            ...
            // Update value
            record[field] = value;
            this.frm.refresh_field('periodontal_records');
        };

        // Bind input events
        $('body').on('change', '.probing-depth', (e) => {
            console.log('changing probing')
    ...
    showToothPeriodontalDialog() {
        const toothRecord = (this.frm.doc.dental_conditions || []).find(t => t.tooth == String(this.selectedTooth)) || {};
    ...
    saveToothRecord(toothNumber, values) {
        ...
        // Find or create record
        let toothRecord = this.frm.doc.dental_conditions.find(t => t.tooth == toothNumber);

        if (!toothRecord) {
            toothRecord = this.frm.add_child('dental_conditions', {
                tooth: toothNumber
            });
        }
        ...
        Object.assign(toothRecord, values);
        this.frm.refresh_field('dental_conditions');
