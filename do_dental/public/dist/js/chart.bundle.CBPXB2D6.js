...
- For vite: pass it via @vitejs/plugin-vue options. See https://github.com/vitejs/vite-plugin-vue/tree/main/packages/plugin-vue#example-for-passing-options-to-vuecompiler-sfc`;
  ...
  var ToothCanvas_default = {
    ...
    emits: ["select", "drag-start", "drag-hover", "drag-end"],
    setup(__props, { expose: __expose, emit: __emit }) {
      ...
      function handleMouseEnter(event) {
        if (!event.buttons)
          return;
        emit2("drag-hover", props.toothNumber);
      }
      ...
      const __returned__ = { emit: emit2, props, canvas, getToothType, resolveToothImage, getOverlaySVG, loadSVGWithColor, computeHorizontal, computeLinePoints, drawPointsCurve, drawGingivalMargin, drawProbingDepth, drawProbingDepthArea, drawTooth, handleMouseDown, handleMouseEnter, handleMouseUp, onMounted, ref, watch: watch2, get overlayMap() {
        return overlayMap;
      }, get surfaceMap() {
        return surfaceMap;
      }, get overlayColors() {
  ...
  var _hoisted_1 = ["onMousedown", "onMouseup"];
  var _hoisted_2 = {
    key: 0,
    class: "number"
  };
  var _hoisted_3 = { ref: "canvas" };
  var _hoisted_4 = {
    key: 1,
    class: "number"
  };
  function render(_ctx, _cache, $props, $setup, $data, $options) {
    return openBlock(), createElementBlock("div", {
      ...
      onMousedown: withModifiers($setup.handleMouseDown, ["left", "stop", "prevent"]),
      onMouseenter: $setup.handleMouseEnter,
      onMouseup: withModifiers($setup.handleMouseUp, ["left", "stop"])
    }, [
      $props.showNum == "up" ? (openBlock(), createElementBlock("span", _hoisted_2, toDisplayString($props.toothNumber), 1)) : createCommentVNode("v-if", true),
      createBaseVNode("canvas", _hoisted_3, null, 512),
      $props.showNum == "down" ? (openBlock(), createElementBlock("span", _hoisted_4, toDisplayString($props.toothNumber), 1)) : createCommentVNode("v-if", true)
    ], 42, _hoisted_1);
  }
  ...
  var STANDARD_PRICE_LIST = "Standard Selling";
  ...
  var DentalChart_default = {
    ...
    setup(__props, { expose: __expose }) {
      ...
      const currentAppointmentId = computed2(
        () => {
          var _a, _b;
          return ((_a = activeAppointment.value) == null ? void 0 : _a.name) || ((_b = activePatientContext.value) == null ? void 0 : _b.appointment) || null;
        }
      );
      const collapsedAppointments = ref([]);
      const groupedVisitHistory = computed2(() => {
        const groups = [];
        ...
        (visitHistory.value || []).forEach((visit) => {
          ...
          if (!summary.date && (visit.appointment_date || visit.date))
            summary.date = visit.appointment_date || visit.date;
          if (!summary.time && (visit.appointment_time || visit.time))
            summary.time = visit.appointment_time || visit.time;
          if (!summary.provider && (visit.appointment_practitioner_name || visit.appointment_practitioner || visit.practitioner_name || visit.practitioner)) {
            summary.provider = visit.appointment_practitioner_name || visit.appointment_practitioner || visit.practitioner_name || visit.practitioner;
          }
          if (summary.billing_amount === null && visit.billing_amount !== void 0) {
            summary.billing_amount = toNumber3(visit.billing_amount, 0);
          }
          if (summary.paid_amount === null && visit.paid_amount !== void 0) {
            summary.paid_amount = toNumber3(visit.paid_amount, 0);
          }
          if (summary.outstanding_amount === null && visit.outstanding_amount !== void 0) {
            summary.outstanding_amount = toNumber3(visit.outstanding_amount, 0);
          }
          if (!summary.currency && visit.currency)
            summary.currency = visit.currency;
          if (!summary.status) {
            summary.status = visit.appointment_custom_status || visit.appointment_status || visit.status || "";
          }
          if (visit.status)
            group.statuses.add(visit.status);
        });
      ...
      function createToothLayout(upperLeft2, upperRight2, lowerLeft2, lowerRight2) {
        const upperArch = [...upperLeft2, ...upperRight2];
        const lowerArch = [...lowerLeft2, ...lowerRight2];
        return {
          upperLeft: upperLeft2,
          upperRight: upperRight2,
          lowerLeft: lowerLeft2,
          lowerRight: lowerRight2,
          upperArch,
          lowerArch,
      ...
      const isPediatricChart = computed2(() => {
        var _a;
        return Boolean((_a = teethData.value.chart) == null ? void 0 : _a.is_pediatric);
      });
      const activeToothLayout = computed2(
        () => isPediatricChart.value ? TOOTH_LAYOUTS.pediatric : TOOTH_LAYOUTS.permanent
      );
      const upperLeft = computed2(() => activeToothLayout.value.upperLeft);
      const upperRight = computed2(() => activeToothLayout.value.upperRight);
      const lowerLeft = computed2(() => activeToothLayout.value.lowerLeft);
      const lowerRight = computed2(() => activeToothLayout.value.lowerRight);
      const ALL_TOOTH_ORDER = computed2(() => activeToothLayout.value.all);
      const upperArchOrder = computed2(() => activeToothLayout.value.upperArch);
      const lowerArchOrder = computed2(() => activeToothLayout.value.lowerArch);
      const SITE_INFO = {
      ...
      const isProcedureMode = computed2(() => selectedAction.value === "procedure");
      const isOverviewMode = computed2(() => selectedAction.value === "overview");
      ...
      const canSubmitProcedure = computed2(() => {
        if (!isProcedureMode.value || !procedurePanelVisible.value)
          return false;
        if (!selectedTeeth.value.length || !selectedProcedureTemplate.value)
          return false;
      ...
      const hasActivePanel = computed2(() => {
        if (isPeriodontalMode.value && periodontalPanelVisible.value && selectedTooth.value)
          return true;
        if (isPathologyMode.value && pathologyPanelVisible.value && pathologyTooth.value)
          return true;
        if (isRestorationMode.value && restorationPanelVisible.value && restorationTooth.value)
          return true;
        if (isConditionMode.value && conditionPanelVisible.value && conditionTooth.value)
          return true;
        if (isProcedureMode.value && procedurePanelVisible.value)
          return true;
        if (isOverviewMode.value && overviewTooth.value)
          return true;
        return false;
      });
      const overviewCondition = computed2(
        () => overviewTooth.value ? getToothCondition(overviewTooth.value) : null
      );
      const overviewRestorations = computed2(
        () => overviewTooth.value ? getToothRestorations(overviewTooth.value) : []
      );
      const overviewPathology = computed2(
        () => overviewTooth.value ? getToothPathology(overviewTooth.value) : []
      );
      const overviewPeriodontal = computed2(
        () => overviewTooth.value ? getPeriodontal(overviewTooth.value) : null
      );
      const selectedTeethSummary = computed2(() => {
        if (!selectedTeeth.value.length)
      ...
      const filteredProcedureTemplates = computed2(() => {
        if (!procedureTemplateSearch.value)
          return procedureTemplates.value;
        const term = procedureTemplateSearch.value.toLowerCase();
        return procedureTemplates.value.filter((template) => {
          return [template.template, template.description, template.item_group].filter(Boolean).some((field) => field.toLowerCase().includes(term));
        });
      });
      function visitHasChanges(visit) {
        var _a, _b;
        const restorations = ((_a = visit == null ? void 0 : visit.changes) == null ? void 0 : _a.restorations) || [];
        const pathology = ((_b = visit == null ? void 0 : visit.changes) == null ? void 0 : _b.pathology) || [];
      ...
      async function loadAppointmentDetails(appointmentId) {
        ...
        try {
          ...
          return (response == null ? void 0 : response.message) || null;
        } catch (err) {
      ...
      function applyProcedureAppointmentDefaults() {
        var _a;
        if (!procedureForm.provider && ((_a = activeAppointment.value) == null ? void 0 : _a.practitioner)) {
      ...
      function readActivePatientContext() {
        var _a, _b;
        try {
          const reader = (_b = (_a = window == null ? void 0 : window.do_health) == null ? void 0 : _a.patientWatcher) == null ? void 0 : _b.read;
      ...
      function findVisitFromContext(context) {
        ...
        const targets = [];
        const encounter = context.patient_encounter || context.encounter || context.encounter_name || context.encounter_id;
        ...
        if (encounter)
          targets.push(`Patient Encounter|${encounter}`);
        if (procedure)
          targets.push(`Clinical Procedure|${procedure}`);
        if (visitId)
          targets.push(visitId);
        return visitHistory.value.find(
          (visit) => targets.includes(visit.id) && (!appointment || visit.appointment === appointment)
        ) || null;
      }
      const currentVisitId = computed2(() => {
        var _a;
        if (!visitHistory.value.length)
          return null;
        const context = activePatientContext.value;
        if ((context == null ? void 0 : context.patient) && context.patient === currentPatientId.value) {
          const matched = findVisitFromContext(context);
          if (matched == null ? void 0 : matched.id)
            return matched.id;
        }
        const active = visitHistory.value.find(
          (entry) => CURRENT_VISIT_STATUSES.includes(String((entry == null ? void 0 : entry.status) || "").toLowerCase())
        );
        if (active == null ? void 0 : active.id)
          return active.id;
        return ((_a = visitHistory.value[0]) == null ? void 0 : _a.id) || null;
      });
      function isCurrentVisit(visit) {
        console.log(visit.appointment);
        const raw = localStorage.getItem("do_health_active_patient");
        if (!(visit == null ? void 0 : visit.appointment))
          return false;
        return visit.appointment === JSON.parse(raw).appointment;
      }
      function visitRowClass(visit) {
        return {
          expanded: isVisitExpanded(visit == null ? void 0 : visit.id),
      ...
      function isCurrentAppointment(group) {
        if (!group)
          return false;
        if (!group.appointment || !currentAppointmentId.value)
          return false;
      ...
      function formatVisitTimestamp(date, time) {
        ...
        if (date) {
          try {
            if (typeof frappe !== "undefined" && ((_a = frappe.datetime) == null ? void 0 : _a.str_to_user)) {
        ...
        if (time) {
          try {
            if (typeof frappe !== "undefined" && ((_b = frappe.datetime) == null ? void 0 : _b.time_to_user)) {
        ...
        return output.join(" \xB7 ");
      }
      function formatPeriodontalSummary(group) {
        if (!group)
          return "No data";
        const values = Object.values(group || {}).map((site) => Number(site == null ? void 0 : site.depth)).filter((value) => Number.isFinite(value));
        if (!values.length)
          return "No data";
        const avg = values.reduce((sum, value) => sum + value, 0) / values.length;
        const max = Math.max(...values);
        return `Avg ${avg.toFixed(1)} \xB7 Max ${max.toFixed(0)}`;
      }
      function openVisitDocument(visit) {
        if (!(visit == null ? void 0 : visit.reference_doctype) || !(visit == null ? void 0 : visit.reference_name))
          return;
        if (typeof (frappe == null ? void 0 : frappe.set_route) === "function") {
          frappe.set_route("Form", visit.reference_doctype, visit.reference_name);
        }
      }
      function openInvoice(invoice) {
        if (!(invoice == null ? void 0 : invoice.name))
          return;
        if (typeof (frappe == null ? void 0 : frappe.set_route) === "function") {
      ...
      async function markVisitComplete(visit) {
        ...
        try {
          ...
          if (typeof (frappe == null ? void 0 : frappe.show_alert) === "function") {
            frappe.show_alert({
              message: (err == null ? void 0 : err.message) || "Could not mark visit complete.",
              indicator: "orange"
            });
          } else {
            window.alert((err == null ? void 0 : err.message) || "Could not mark visit complete.");
          }
        } finally {
      ...
      function createInvoiceForVisit(visit) {
        ...
        frappe.route_options = {
          ...
          reference_dt: visit == null ? void 0 : visit.reference_doctype,
          reference_dn: visit == null ? void 0 : visit.reference_name
        };
      ...
      async function deleteVisit(visit) {
        if (!(visit == null ? void 0 : visit.reference_doctype) || !(visit == null ? void 0 : visit.reference_name))
          return;
        if (!window.confirm("Delete this visit? Related chart updates will also be removed."))
          return;
        try {
          ...
          if (typeof (frappe == null ? void 0 : frappe.msgprint) === "function") {
            frappe.msgprint({
              message: (err == null ? void 0 : err.message) || "Could not delete visit.",
              title: "Delete Visit",
              indicator: "red"
            });
          } else {
            window.alert((err == null ? void 0 : err.message) || "Could not delete visit.");
          }
        } finally {
      ...
      function handleToothDragStart(payload = {}) {
        if (!isProcedureMode.value) {
          if (payload == null ? void 0 : payload.tooth)
            handleToothSelect(payload.tooth);
          return;
        }
      ...
      function handleToothDragHover(tooth) {
        if (!dragSelectionState.active || !isProcedureMode.value)
          return;
      ...
      function detectRestorationType(template) {
        const target = ((template == null ? void 0 : template.template) || (template == null ? void 0 : template.name) || "").toLowerCase();
        if (target.includes("crown"))
          return "Crown";
        if (target.includes("veneer"))
          return "Veneer";
        if (target.includes("onlay"))
          return "Onlay";
        if (target.includes("inlay"))
          return "Inlay";
        if (target.includes("bridge"))
          return "Partial Crown";
        return "Filling";
      }
      function initProviderLinkControl() {
        var _a;
        if (providerLinkControl.value || !procedureProviderRef.value)
          return;
        providerLinkControl.value = frappe.ui.form.make_control({
          ...
          df: {
            ...
            change() {
              var _a2;
              const next = ((_a2 = providerLinkControl.value) == null ? void 0 : _a2.get_value()) || "";
        ...
        (_a = providerLinkControl.value) == null ? void 0 : _a.set_input(procedureForm.provider || "");
      }
      function destroyProviderLinkControl() {
        var _a, _b;
        if (!providerLinkControl.value)
          return;
        try {
          (_b = (_a = providerLinkControl.value) == null ? void 0 : _a.$wrapper) == null ? void 0 : _b.remove();
        } catch (err) {
      ...
      function syncProviderControlValue() {
        ...
        if (!providerLinkControl.value)
          return;
        const currentValue = ((_b = (_a = providerLinkControl.value).get_value) == null ? void 0 : _b.call(_a)) || "";
      ...
      function ensureProcedureTemplates(force = false) {
        if (procedureTemplatesLoaded.value && !force)
          return;
      ...
      async function fetchStandardPriceMap(itemCodes) {
        ...
        try {
          ...
          const map2 = {};
          ((response == null ? void 0 : response.message) || []).forEach((row) => {
            var _a;
            const code = row.item_code;
            if (!code || map2[code])
              return;
            map2[code] = {
              rate: (_a = row.price_list_rate) != null ? _a : null,
      ...
      async function loadProcedureTemplates() {
        if (procedureTemplatesLoading.value)
          return;
        ...
        try {
          ...
          procedureTemplates.value = baseTemplates.map((entry) => {
            ...
            return __spreadProps(__spreadValues({}, entry), {
              standard_price: (_a = price == null ? void 0 : price.rate) != null ? _a : null,
              standard_price_currency: (price == null ? void 0 : price.currency) || null,
              standard_price_list: (price == null ? void 0 : price.price_list) || null
            });
          });
      ...
      function buildProcedureRestorationPayload() {
        ...
        if ((_a = procedureForm.surfaces) == null ? void 0 : _a.length) {
      ...
      async function submitProcedure() {
        var _a;
        if (!canSubmitProcedure.value || !currentPatientId.value)
          return;
        savingProcedure.value = true;
        procedureError.value = null;
        try {
          const appointment = activeAppointment.value;
          const provider = procedureForm.provider || (appointment == null ? void 0 : appointment.practitioner) || null;
          const payload = {
            patient_id: currentPatientId.value,
            template: ((_a = selectedProcedureTemplate.value) == null ? void 0 : _a.name) || null,
            teeth: selectedTeeth.value,
            restoration: buildProcedureRestorationPayload(),
            procedure: {
              provider,
              appointment: (appointment == null ? void 0 : appointment.name) || null,
              company: (appointment == null ? void 0 : appointment.company) || null,
              date: (appointment == null ? void 0 : appointment.appointment_date) || null,
              time: (appointment == null ? void 0 : appointment.appointment_time) || null,
          ...
          procedureError.value = (err == null ? void 0 : err.message) || "Could not save procedure. Please try again.";
      ...
      async function syncActiveAppointmentContext(context) {
        ...
        if (((_a = activeAppointment.value) == null ? void 0 : _a.name) === appointmentId) {
          applyProcedureAppointmentDefaults();
          return;
        }
      ...
      function assignTeethData(payload) {
        ...
        visitHistory.value = (payload == null ? void 0 : payload.visits) || [];
      ...
      onMounted(() => {
        ...
        const subscriber = (_b = (_a = window == null ? void 0 : window.do_health) == null ? void 0 : _a.patientWatcher) == null ? void 0 : _b.subscribe;
      ...
      const __returned__ = { props, DEFAULT_PATIENT_ID, EMPTY_ARRAY, PATHOLOGY_TYPE_OPTIONS, DECAY_STAGE_OPTIONS, CAVITATION_OPTIONS, PULP_OPTIONS, DECAY_LEVEL_OPTIONS, FRACTURE_TYPE_OPTIONS, FRACTURE_POSITION_OPTIONS, WEAR_TYPE_OPTIONS, DISCOLORATION_TYPE_OPTIONS, RESTORATION_TYPE_OPTIONS, RESTORATION_MATERIAL_OPTIONS, RESTORATION_QUALITY_OPTIONS, RESTORATION_DETAIL_OPTIONS, CROWN_MATERIAL_OPTIONS, CROWN_TYPE_OPTIONS, CROWN_BASE_OPTIONS, CROWN_IMPLANT_TYPE_OPTIONS, CONDITION_STATE_OPTIONS, ERUPTION_STATUS_OPTIONS, STANDARD_PRICE_LIST, DEFAULT_SURFACE_OPTIONS, buildSurfaceOptionList, formatSurfaceLabel, actionOptions, CUSTOM_ACTION, selectedAction, view, showLegend, chartExpanded, loading, loadError, currentPatientId, loadErrorMessage, selectedTeeth, dragSelectionState, procedurePanelVisible, procedureTemplates, procedureTemplatesLoaded, procedureTemplatesLoading, procedureTemplateSearch, selectedProcedureTemplate, procedureProviderRef, providerLinkControl, procedureForm, procedureError, savingProcedure, visitHistory, expandedVisits, deletingVisitId, activeVisitActionId, completingVisitId, activePatientContext, activeAppointment, overviewTooth, surfaceOptions, surfaceOptionsLoaded, surfacesLoading, currentAppointmentId, collapsedAppointments, groupedVisitHistory, pathologyTooth, pathologyPanelVisible, pathologyEditId, pathologyForm, pathologyError, savingPathology, restorationTooth, restorationPanelVisible, restorationEditId, restorationForm, restorationError, savingRestoration, conditionTooth, conditionPanelVisible, conditionForm, conditionError, savingCondition, savingPeriodontal, periodontalSaveMessage, layerOptions, activeLayers, CURRENT_VISIT_STATUSES, get patientWatcherUnsub() {
        return patientWatcherUnsub;
      }, set patientWatcherUnsub(v) {
        patientWatcherUnsub = v;
      }, ACTIVE_PATIENT_KEYS, APPOINTMENT_FIELDS, teethData, createToothLayout, TOOTH_LAYOUTS, isPediatricChart, activeToothLayout, upperLeft, upperRight, lowerLeft, lowerRight, ALL_TOOTH_ORDER, upperArchOrder, lowerArchOrder, SITE_INFO, PANEL_SITE_LAYOUT, PERIODONTAL_FLAGS, periodontalAutoAdvance, periodontalCursor, periodontalQueue, currentQueueEntry, selectedTooth, currentPeriodontalSite, periodontalPanelVisible, layerIsActive, isPeriodontalMode, isPathologyMode, isRestorationMode, isConditionMode, isProcedureMode, isOverviewMode, canSubmitPathology, canSubmitRestoration, canSubmitCondition, canSubmitProcedure, showRestorationDetailFields, showCrownFields, showImplantFields, activePathologyEntries, activeRestorationEntries, activeConditionEntry, hasActivePanel, overviewCondition, overviewRestorations, overviewPathology, overviewPeriodontal, selectedTeethSummary, filteredProcedureTemplates, visitHasChanges, normalizeActivePatient, loadAppointmentDetails, applyProcedureAppointmentDefaults, readActivePatientContext, findVisitFromContext, currentVisitId, isCurrentVisit, visitRowClass, groupRowClass, isVisitExpanded, isAppointmentCollapsed, toggleAppointmentGroup, isCurrentAppointment, toggleVisitDetails, formatCurrency, formatVisitTimestamp, formatPeriodontalSummary, openVisitDocument, openInvoice, toggleVisitActions, markVisitComplete, createInvoiceForVisit, deleteVisit, periodontalPanelRef, handlePatientChange, isSelectedTooth, handleToothSelect, ensureUniqueTeeth, addToothSelection, removeToothSelection, clearSelectedTeeth, clearOverviewTooth, resetOverviewPanel, handleProcedureSelection, handleToothDragStart, handleToothDragHover, handleToothDragEnd, resetProcedureForm, closeProcedurePanel, detectRestorationType, initProviderLinkControl, destroyProviderLinkControl, syncProviderControlValue, ensureProcedureTemplates, fetchStandardPriceMap, loadProcedureTemplates, selectProcedureTemplate, buildProcedureRestorationPayload, submitProcedure, periodontalToothSequence, buildPeriodontalQueue, panelSites, isUpperToothNumber, siteDisplayLabel, isActiveSite, hidePeriodontalPanel, handlePeriodontalKeyDown, ensureSurfaceOptions, syncActiveAppointmentContext, toothEventHandlers, startPathologyFlow, resetPathologyForm, populatePathologyForm, editPathology, closePathologyPanel, startRestorationFlow, resetRestorationForm, populateRestorationForm, editRestoration, closeRestorationPanel, startConditionFlow, resetConditionForm, closeConditionPanel, convertSurfaceSelection, surfaceListFromEntry, buildPathologyPayload, buildRestorationPayload, buildConditionPayload, appendLocalPathology, appendLocalRestoration, removeLocalPathology, removeLocalRestoration, updateLocalCondition, joinSurfaceLabels, summarisePathology, summariseRestoration, summariseCondition, toggleChartExpansion, deleteChartRecord, deletePathology, deleteRestoration, submitPathology, submitRestoration, submitCondition, selectAction, toggleLayer, layersMatch, normaliseLayerList, normaliseSurfaceList, normalisePathologyEntry, normaliseRestorationEntry, toNumber: toNumber3, toNullableInt, createLocalId, createEmptySite, mergeSite, createDefaultPeriodontal, deepFreeze, DEFAULT_PERIODONTAL_ENTRY, ensurePeriodontalEntry, conditionsByTooth, pathologyByTooth, restorationsByTooth, periodontalByTooth, currentPeriodontalMeasurement, hasPreviousTooth, hasNextTooth, getToothCondition, getToothPathology, getToothRestorations, getPeriodontal, getNeighborNumber, getNeighborPeriodontal, selectPeriodontalSite, goToPreviousTooth, goToNextTooth, goToNextSite, goToPreviousSite, updatePeriodontalSiteValue, setCurrentSiteMeasurement, toggleCurrentSiteFlag, confirmPeriodontalEntry, buildPeriodontalTestPayload, submitPeriodontalTest, periodontalSiteSummary, assignTeethData, clearChartData, loadChart, computed: computed2, nextTick, onBeforeUnmount, onMounted, reactive, ref, watch: watch2, ToothCanvas: ToothCanvas_default2, get surfaceMap() {
        return surfaceMap;
      } };
      Object.defineProperty(__returned__, "__isScriptSetup", { enumerable: false, value: true });
      return __returned__;
  ...
  var _hoisted_12 = { class: "chart-header" };
  var _hoisted_22 = /* @__PURE__ */ _withScopeId(() => /* @__PURE__ */ createBaseVNode("div", { class: "chart-title" }, [
    /* @__PURE__ */ createBaseVNode("h2", null, "Dental Chart"),
    /* @__PURE__ */ createCommentVNode(' <p class="chart-hint">Select a tooth to open the work area.</p> ')
  ], -1));
  var _hoisted_32 = { class: "chart-meta" };
  var _hoisted_42 = {
    key: 0,
    class: "chart-status"
  };
  ...
  var _hoisted_6 = { class: "chart-main" };
  var _hoisted_7 = { class: "chart-body" };
  var _hoisted_8 = {
    key: 0,
    class: "all-jaws"
  };
  var _hoisted_9 = { class: "teeth-row" };
  var _hoisted_10 = { class: "teeth-row" };
  var _hoisted_11 = { class: "teeth-row" };
  var _hoisted_122 = { class: "teeth-row" };
  var _hoisted_13 = { class: "upper-jaw" };
  var _hoisted_14 = { class: "teeth-row" };
  var _hoisted_15 = { class: "teeth-row" };
  var _hoisted_16 = { class: "teeth-row" };
  var _hoisted_17 = { class: "lower-jaw" };
  var _hoisted_18 = { class: "teeth-row" };
  var _hoisted_19 = { class: "teeth-row" };
  var _hoisted_20 = { class: "teeth-row" };
  var _hoisted_21 = { class: "mouth" };
  var _hoisted_222 = /* @__PURE__ */ _withScopeId(() => /* @__PURE__ */ createBaseVNode("svg", {
    width: "28",
    height: "28",
    viewBox: "0 0 28 28",
  ...
  var _hoisted_24 = /* @__PURE__ */ _withScopeId(() => /* @__PURE__ */ createBaseVNode("svg", {
    width: "28",
    height: "28",
    viewBox: "0 0 28 28",
  ...
  var _hoisted_26 = /* @__PURE__ */ _withScopeId(() => /* @__PURE__ */ createBaseVNode("svg", {
    width: "28",
    height: "28",
    viewBox: "0 0 28 28",
  ...
  var _hoisted_28 = { class: "layer-controls" };
  var _hoisted_29 = ["checked", "onChange"];
  var _hoisted_30 = { class: "workflow-stack" };
  var _hoisted_31 = { class: "actions-card" };
  var _hoisted_322 = { class: "action-tabs" };
  ...
  var _hoisted_36 = { class: "overview-header" };
  var _hoisted_37 = /* @__PURE__ */ _withScopeId(() => /* @__PURE__ */ createBaseVNode("h4", null, "Tooth Overview", -1));
  var _hoisted_38 = { class: "overview-grid" };
  var _hoisted_39 = /* @__PURE__ */ _withScopeId(() => /* @__PURE__ */ createBaseVNode("h5", null, "Condition", -1));
  var _hoisted_40 = { key: 0 };
  var _hoisted_41 = {
    key: 1,
    class: "field-hint"
  };
  var _hoisted_422 = /* @__PURE__ */ _withScopeId(() => /* @__PURE__ */ createBaseVNode("h5", null, "Restorations", -1));
  var _hoisted_43 = { key: 0 };
  var _hoisted_44 = {
    key: 1,
    class: "field-hint"
  };
  var _hoisted_45 = /* @__PURE__ */ _withScopeId(() => /* @__PURE__ */ createBaseVNode("h5", null, "Pathology", -1));
  var _hoisted_46 = { key: 0 };
  var _hoisted_47 = {
    key: 1,
    class: "field-hint"
  };
  var _hoisted_48 = /* @__PURE__ */ _withScopeId(() => /* @__PURE__ */ createBaseVNode("h5", null, "Periodontal", -1));
  var _hoisted_49 = {
    key: 0,
    class: "periodontal-summary"
  };
  var _hoisted_50 = /* @__PURE__ */ _withScopeId(() => /* @__PURE__ */ createBaseVNode("span", { class: "label" }, "Buccal", -1));
  var _hoisted_51 = /* @__PURE__ */ _withScopeId(() => /* @__PURE__ */ createBaseVNode("span", { class: "label" }, "Lingual", -1));
  var _hoisted_52 = {
    key: 1,
    class: "field-hint"
  };
  var _hoisted_53 = {
    key: 1,
    class: "action-work-area"
  };
  var _hoisted_54 = { class: "panel-header" };
  var _hoisted_55 = ["disabled"];
  var _hoisted_56 = { class: "panel-title" };
  var _hoisted_57 = /* @__PURE__ */ _withScopeId(() => /* @__PURE__ */ createBaseVNode("span", { class: "label" }, "Tooth", -1));
  var _hoisted_58 = { class: "panel-actions" };
  var _hoisted_59 = ["disabled"];
  var _hoisted_60 = { class: "site-grid" };
  var _hoisted_61 = ["onClick"];
  var _hoisted_62 = { class: "site-label" };
  var _hoisted_63 = { class: "site-summary" };
  var _hoisted_64 = { class: "measurement-section" };
  var _hoisted_65 = /* @__PURE__ */ _withScopeId(() => /* @__PURE__ */ createBaseVNode("span", null, "Probing depth (0-9 mm)", -1));
  var _hoisted_66 = ["value", "onKeyup"];
  var _hoisted_67 = /* @__PURE__ */ _withScopeId(() => /* @__PURE__ */ createBaseVNode("span", null, "Gingival margin (mm)", -1));
  var _hoisted_68 = ["value", "onKeyup"];
  var _hoisted_69 = { class: "flags-section" };
  var _hoisted_70 = ["checked", "onChange"];
  var _hoisted_71 = { class: "panel-footer" };
  var _hoisted_72 = { class: "auto-toggle" };
  var _hoisted_73 = /* @__PURE__ */ _withScopeId(() => /* @__PURE__ */ createBaseVNode("span", null, "Automatic sequential probing", -1));
  var _hoisted_74 = { class: "footer-actions" };
  ...
  var _hoisted_78 = { class: "panel-header" };
  var _hoisted_79 = /* @__PURE__ */ _withScopeId(() => /* @__PURE__ */ createBaseVNode("h3", { class: "panel-title" }, "Tooth Condition", -1));
  var _hoisted_80 = { class: "panel-subtitle" };
  var _hoisted_81 = { class: "panel-body" };
  var _hoisted_82 = {
    key: 0,
    class: "existing-list"
  };
  var _hoisted_83 = /* @__PURE__ */ _withScopeId(() => /* @__PURE__ */ createBaseVNode("h4", null, "Existing", -1));
  var _hoisted_84 = { class: "form-field" };
  var _hoisted_85 = /* @__PURE__ */ _withScopeId(() => /* @__PURE__ */ createBaseVNode("label", { for: "condition-state" }, "State", -1));
  ...
  var _hoisted_89 = /* @__PURE__ */ _withScopeId(() => /* @__PURE__ */ createBaseVNode("label", { for: "eruption-status" }, "Eruption Status", -1));
  var _hoisted_90 = /* @__PURE__ */ _withScopeId(() => /* @__PURE__ */ createBaseVNode("option", { value: "" }, "Select", -1));
  var _hoisted_91 = ["value"];
  var _hoisted_92 = { class: "form-field" };
  var _hoisted_93 = /* @__PURE__ */ _withScopeId(() => /* @__PURE__ */ createBaseVNode("label", { for: "condition-mobility" }, "Mobility", -1));
  var _hoisted_94 = { class: "form-field" };
  var _hoisted_95 = /* @__PURE__ */ _withScopeId(() => /* @__PURE__ */ createBaseVNode("label", { for: "condition-furcation" }, "Furcation", -1));
  var _hoisted_96 = { class: "form-field" };
  var _hoisted_97 = /* @__PURE__ */ _withScopeId(() => /* @__PURE__ */ createBaseVNode("label", { for: "condition-notes" }, "Notes", -1));
  var _hoisted_98 = {
    key: 1,
    class: "panel-error"
  };
  var _hoisted_99 = { class: "panel-footer" };
  var _hoisted_100 = ["disabled"];
  var _hoisted_101 = {
    key: 0,
    class: "action-panel pathology-panel"
  };
  var _hoisted_102 = { class: "panel-header" };
  var _hoisted_103 = /* @__PURE__ */ _withScopeId(() => /* @__PURE__ */ createBaseVNode("h3", { class: "panel-title" }, "Pathology", -1));
  var _hoisted_104 = { class: "panel-subtitle" };
  var _hoisted_105 = { class: "panel-body" };
  var _hoisted_106 = {
    key: 0,
    class: "existing-list"
  };
  var _hoisted_107 = /* @__PURE__ */ _withScopeId(() => /* @__PURE__ */ createBaseVNode("h4", null, "Existing", -1));
  var _hoisted_108 = { class: "item-actions" };
  var _hoisted_109 = ["onClick"];
  var _hoisted_110 = ["onClick"];
  var _hoisted_111 = { class: "form-field" };
  var _hoisted_112 = /* @__PURE__ */ _withScopeId(() => /* @__PURE__ */ createBaseVNode("label", { for: "pathology-type" }, "Pathology Type", -1));
  ...
  var _hoisted_117 = /* @__PURE__ */ _withScopeId(() => /* @__PURE__ */ createBaseVNode("label", { for: "decay-stage" }, "Decay Stage", -1));
  ...
  var _hoisted_121 = /* @__PURE__ */ _withScopeId(() => /* @__PURE__ */ createBaseVNode("label", { for: "decay-cavitation" }, "Cavitation", -1));
  var _hoisted_1222 = /* @__PURE__ */ _withScopeId(() => /* @__PURE__ */ createBaseVNode("option", { value: "" }, "Select", -1));
  var _hoisted_123 = ["value"];
  var _hoisted_124 = {
    key: 0,
    class: "form-field"
  };
  var _hoisted_125 = /* @__PURE__ */ _withScopeId(() => /* @__PURE__ */ createBaseVNode("label", { for: "decay-pulp" }, "Pulp", -1));
  var _hoisted_126 = /* @__PURE__ */ _withScopeId(() => /* @__PURE__ */ createBaseVNode("option", { value: "" }, "Select", -1));
  var _hoisted_127 = ["value"];
  var _hoisted_128 = { class: "form-field" };
  var _hoisted_129 = /* @__PURE__ */ _withScopeId(() => /* @__PURE__ */ createBaseVNode("label", { for: "decay-level" }, "Decay Level", -1));
  var _hoisted_130 = /* @__PURE__ */ _withScopeId(() => /* @__PURE__ */ createBaseVNode("option", { value: "" }, "Select", -1));
  var _hoisted_131 = ["value"];
  var _hoisted_132 = { class: "form-field" };
  var _hoisted_133 = /* @__PURE__ */ _withScopeId(() => /* @__PURE__ */ createBaseVNode("label", null, "Decay Surfaces", -1));
  var _hoisted_134 = {
    key: 0,
    class: "surface-grid"
  };
  ...
  var _hoisted_139 = { class: "form-field" };
  var _hoisted_140 = /* @__PURE__ */ _withScopeId(() => /* @__PURE__ */ createBaseVNode("label", { for: "fracture-type" }, "Fracture Type", -1));
  var _hoisted_141 = /* @__PURE__ */ _withScopeId(() => /* @__PURE__ */ createBaseVNode("option", { value: "" }, "Select", -1));
  var _hoisted_142 = ["value"];
  var _hoisted_143 = { class: "form-field" };
  var _hoisted_144 = /* @__PURE__ */ _withScopeId(() => /* @__PURE__ */ createBaseVNode("label", { for: "fracture-position" }, "Fracture Position", -1));
  var _hoisted_145 = /* @__PURE__ */ _withScopeId(() => /* @__PURE__ */ createBaseVNode("option", { value: "" }, "Select", -1));
  var _hoisted_146 = ["value"];
  var _hoisted_147 = {
    key: 3,
    class: "form-section"
  };
  var _hoisted_148 = { class: "form-field" };
  var _hoisted_149 = /* @__PURE__ */ _withScopeId(() => /* @__PURE__ */ createBaseVNode("label", { for: "wear-type" }, "Wear Type", -1));
  var _hoisted_150 = /* @__PURE__ */ _withScopeId(() => /* @__PURE__ */ createBaseVNode("option", { value: "" }, "Select", -1));
  var _hoisted_151 = ["value"];
  var _hoisted_152 = { class: "form-field" };
  var _hoisted_153 = /* @__PURE__ */ _withScopeId(() => /* @__PURE__ */ createBaseVNode("label", null, "Wear Surfaces", -1));
  var _hoisted_154 = {
    key: 0,
    class: "surface-grid"
  };
  ...
  var _hoisted_159 = { class: "form-field" };
  var _hoisted_160 = /* @__PURE__ */ _withScopeId(() => /* @__PURE__ */ createBaseVNode("label", { for: "discoloration-type" }, "Discoloration Type", -1));
  var _hoisted_161 = /* @__PURE__ */ _withScopeId(() => /* @__PURE__ */ createBaseVNode("option", { value: "" }, "Select", -1));
  var _hoisted_162 = ["value"];
  var _hoisted_163 = { class: "form-field" };
  var _hoisted_164 = /* @__PURE__ */ _withScopeId(() => /* @__PURE__ */ createBaseVNode("label", { for: "pathology-notes" }, "Notes", -1));
  var _hoisted_165 = {
    key: 5,
    class: "panel-error"
  };
  var _hoisted_166 = { class: "panel-footer" };
  var _hoisted_167 = ["disabled"];
  var _hoisted_168 = {
    key: 0,
    class: "action-panel restoration-panel"
  };
  var _hoisted_169 = { class: "panel-header" };
  var _hoisted_170 = /* @__PURE__ */ _withScopeId(() => /* @__PURE__ */ createBaseVNode("h3", { class: "panel-title" }, "Restoration", -1));
  var _hoisted_171 = { class: "panel-subtitle" };
  var _hoisted_172 = { class: "panel-body" };
  var _hoisted_173 = {
    key: 0,
    class: "existing-list"
  };
  var _hoisted_174 = /* @__PURE__ */ _withScopeId(() => /* @__PURE__ */ createBaseVNode("h4", null, "Existing", -1));
  var _hoisted_175 = { class: "item-actions" };
  var _hoisted_176 = ["onClick"];
  var _hoisted_177 = ["onClick"];
  var _hoisted_178 = { class: "form-field" };
  var _hoisted_179 = /* @__PURE__ */ _withScopeId(() => /* @__PURE__ */ createBaseVNode("label", { for: "restoration-type" }, "Restoration Type", -1));
  var _hoisted_180 = /* @__PURE__ */ _withScopeId(() => /* @__PURE__ */ createBaseVNode("option", { value: "" }, "Select", -1));
  var _hoisted_181 = ["value"];
  var _hoisted_182 = { class: "form-field" };
  var _hoisted_183 = /* @__PURE__ */ _withScopeId(() => /* @__PURE__ */ createBaseVNode("label", { for: "restoration-material" }, "Material", -1));
  var _hoisted_184 = /* @__PURE__ */ _withScopeId(() => /* @__PURE__ */ createBaseVNode("option", { value: "" }, "Select", -1));
  var _hoisted_185 = ["value"];
  var _hoisted_186 = { class: "form-field" };
  var _hoisted_187 = /* @__PURE__ */ _withScopeId(() => /* @__PURE__ */ createBaseVNode("label", null, "Surfaces", -1));
  var _hoisted_188 = {
    key: 0,
    class: "surface-grid"
  };
  var _hoisted_189 = ["value"];
  var _hoisted_190 = {
    key: 1,
    class: "field-hint"
  };
  ...
  var _hoisted_192 = { class: "form-field" };
  var _hoisted_193 = /* @__PURE__ */ _withScopeId(() => /* @__PURE__ */ createBaseVNode("label", { for: "restoration-quality" }, "Quality", -1));
  var _hoisted_194 = /* @__PURE__ */ _withScopeId(() => /* @__PURE__ */ createBaseVNode("option", { value: "" }, "Select", -1));
  var _hoisted_195 = ["value"];
  var _hoisted_196 = { class: "form-field" };
  var _hoisted_197 = /* @__PURE__ */ _withScopeId(() => /* @__PURE__ */ createBaseVNode("label", { for: "restoration-detail" }, "Detail", -1));
  var _hoisted_198 = /* @__PURE__ */ _withScopeId(() => /* @__PURE__ */ createBaseVNode("option", { value: "" }, "Select", -1));
  var _hoisted_199 = ["value"];
  var _hoisted_200 = { class: "form-field" };
  var _hoisted_201 = /* @__PURE__ */ _withScopeId(() => /* @__PURE__ */ createBaseVNode("label", { for: "crown-material" }, "Crown Material", -1));
  var _hoisted_202 = /* @__PURE__ */ _withScopeId(() => /* @__PURE__ */ createBaseVNode("option", { value: "" }, "Select", -1));
  var _hoisted_203 = ["value"];
  var _hoisted_204 = { class: "form-field" };
  var _hoisted_205 = /* @__PURE__ */ _withScopeId(() => /* @__PURE__ */ createBaseVNode("label", { for: "crown-type" }, "Crown Type", -1));
  var _hoisted_206 = /* @__PURE__ */ _withScopeId(() => /* @__PURE__ */ createBaseVNode("option", { value: "" }, "Select", -1));
  var _hoisted_207 = ["value"];
  var _hoisted_208 = { class: "form-field" };
  var _hoisted_209 = /* @__PURE__ */ _withScopeId(() => /* @__PURE__ */ createBaseVNode("label", { for: "crown-base" }, "Crown Base", -1));
  var _hoisted_210 = /* @__PURE__ */ _withScopeId(() => /* @__PURE__ */ createBaseVNode("option", { value: "" }, "Select", -1));
  var _hoisted_211 = ["value"];
  var _hoisted_212 = {
    key: 0,
    class: "form-field"
  };
  var _hoisted_213 = /* @__PURE__ */ _withScopeId(() => /* @__PURE__ */ createBaseVNode("label", { for: "crown-implant-type" }, "Crown Implant Type", -1));
  var _hoisted_214 = /* @__PURE__ */ _withScopeId(() => /* @__PURE__ */ createBaseVNode("option", { value: "" }, "Select", -1));
  var _hoisted_215 = ["value"];
  var _hoisted_216 = {
    key: 3,
    class: "panel-error"
  };
  var _hoisted_217 = { class: "panel-footer" };
  var _hoisted_218 = ["disabled"];
  var _hoisted_219 = {
    key: 0,
    class: "action-panel procedure-panel"
  };
  var _hoisted_220 = { class: "panel-header" };
  var _hoisted_221 = /* @__PURE__ */ _withScopeId(() => /* @__PURE__ */ createBaseVNode("h3", { class: "panel-title" }, "Clinical Procedure", -1));
  var _hoisted_2222 = {
    key: 0,
  ...
  var _hoisted_224 = { class: "panel-body" };
  var _hoisted_225 = { class: "selection-overview" };
  var _hoisted_226 = /* @__PURE__ */ _withScopeId(() => /* @__PURE__ */ createBaseVNode("span", null, "teeth selected", -1));
  var _hoisted_227 = { class: "form-field" };
  var _hoisted_228 = /* @__PURE__ */ _withScopeId(() => /* @__PURE__ */ createBaseVNode("label", { for: "procedure-search" }, "Search Template", -1));
  var _hoisted_229 = { class: "template-grid" };
  ...
  var _hoisted_236 = { class: "form-field" };
  var _hoisted_237 = /* @__PURE__ */ _withScopeId(() => /* @__PURE__ */ createBaseVNode("label", { for: "procedure-type" }, "Restoration Type", -1));
  var _hoisted_238 = /* @__PURE__ */ _withScopeId(() => /* @__PURE__ */ createBaseVNode("option", { value: "" }, "Select", -1));
  var _hoisted_239 = ["value"];
  var _hoisted_240 = { class: "form-field" };
  var _hoisted_241 = /* @__PURE__ */ _withScopeId(() => /* @__PURE__ */ createBaseVNode("label", { for: "procedure-material" }, "Material", -1));
  var _hoisted_242 = /* @__PURE__ */ _withScopeId(() => /* @__PURE__ */ createBaseVNode("option", { value: "" }, "Select", -1));
  var _hoisted_243 = ["value"];
  var _hoisted_244 = { class: "form-field" };
  var _hoisted_245 = /* @__PURE__ */ _withScopeId(() => /* @__PURE__ */ createBaseVNode("label", null, "Surfaces", -1));
  var _hoisted_246 = {
    key: 0,
    class: "surface-grid"
  };
  var _hoisted_247 = ["value"];
  var _hoisted_248 = {
    key: 1,
    class: "field-hint"
  };
  var _hoisted_249 = { class: "form-field" };
  var _hoisted_250 = /* @__PURE__ */ _withScopeId(() => /* @__PURE__ */ createBaseVNode("label", { for: "procedure-provider" }, "Provider", -1));
  var _hoisted_251 = {
    id: "procedure-provider",
    ref: "procedureProviderRef"
  };
  var _hoisted_252 = { class: "form-field" };
  var _hoisted_253 = /* @__PURE__ */ _withScopeId(() => /* @__PURE__ */ createBaseVNode("label", { for: "procedure-notes" }, "Notes", -1));
  var _hoisted_254 = {
    key: 1,
    class: "panel-error"
  };
  var _hoisted_255 = { class: "panel-footer" };
  var _hoisted_256 = ["disabled"];
  var _hoisted_257 = {
    key: 0,
    class: "panel-placeholder"
  };
  var _hoisted_258 = { class: "history-panel" };
  var _hoisted_259 = /* @__PURE__ */ _withScopeId(() => /* @__PURE__ */ createBaseVNode("div", null, [
    /* @__PURE__ */ createBaseVNode("h3", null, "Visit History")
  ], -1));
  var _hoisted_260 = { class: "history-count" };
  var _hoisted_261 = {
    key: 0,
    class: "history-empty"
  };
  var _hoisted_262 = { class: "history-table-wrapper" };
  var _hoisted_263 = { class: "history-table" };
  var _hoisted_264 = /* @__PURE__ */ _withScopeId(() => /* @__PURE__ */ createBaseVNode("thead", null, [
    /* @__PURE__ */ createBaseVNode("tr", null, [
      /* @__PURE__ */ createBaseVNode("th", null, "Visit"),
      /* @__PURE__ */ createBaseVNode("th", null, "Date"),
      /* @__PURE__ */ createBaseVNode("th", null, "Provider"),
      /* @__PURE__ */ createBaseVNode("th", null, "Status"),
      /* @__PURE__ */ createBaseVNode("th", null, "Billing"),
      /* @__PURE__ */ createBaseVNode("th")
    ])
  ], -1));
  var _hoisted_265 = ["onClick"];
  var _hoisted_266 = { class: "group-title-cell" };
  var _hoisted_267 = { class: "group-header" };
  var _hoisted_268 = { class: "group-label" };
  var _hoisted_269 = { class: "billing-chip compact" };
  var _hoisted_270 = { class: "amount" };
  var _hoisted_271 = { key: 0 };
  var _hoisted_272 = { key: 1 };
  var _hoisted_273 = /* @__PURE__ */ _withScopeId(() => /* @__PURE__ */ createBaseVNode("td", null, null, -1));
  var _hoisted_274 = ["onClick", "onContextmenu"];
  var _hoisted_275 = { class: "visit-title" };
  var _hoisted_276 = { class: "history-purpose" };
  var _hoisted_277 = /* @__PURE__ */ _withScopeId(() => /* @__PURE__ */ createBaseVNode("td", null, null, -1));
  var _hoisted_278 = { class: "actions-cell" };
  var _hoisted_279 = ["onClick"];
  var _hoisted_280 = ["disabled", "onClick"];
  var _hoisted_281 = ["onClick"];
  var _hoisted_282 = ["disabled", "onClick"];
  var _hoisted_283 = {
    key: 0,
    class: "history-details-row"
  };
  var _hoisted_284 = { colspan: "6" };
  var _hoisted_285 = { class: "history-details" };
  var _hoisted_286 = /* @__PURE__ */ _withScopeId(() => /* @__PURE__ */ createBaseVNode("h5", null, "Restorations", -1));
  var _hoisted_287 = /* @__PURE__ */ _withScopeId(() => /* @__PURE__ */ createBaseVNode("h5", null, "Pathology", -1));
  var _hoisted_288 = {
    key: 2,
    class: "invoice-chips"
  };
  var _hoisted_289 = ["onClick"];
  var _hoisted_290 = { class: "legend-panel" };
  var _hoisted_291 = /* @__PURE__ */ _withScopeId(() => /* @__PURE__ */ createBaseVNode("h3", null, "Legend", -1));
  var _hoisted_292 = /* @__PURE__ */ _withScopeId(() => /* @__PURE__ */ createBaseVNode("section", null, [
  ...
  function render2(_ctx, _cache, $props, $setup, $data, $options) {
    return openBlock(), createElementBlock("div", {
      ...
      createBaseVNode("div", {
        class: normalizeClass(["chart-layout", { "chart-expanded": $setup.chartExpanded }])
      }, [
        createBaseVNode("section", {
          class: normalizeClass(["chart-area", { expanded: $setup.chartExpanded }])
        }, [
          createBaseVNode("div", _hoisted_12, [
            _hoisted_22,
            createBaseVNode("div", _hoisted_32, [
              $setup.loading ? (openBlock(), createElementBlock("span", _hoisted_42, "Loading\u2026")) : $setup.loadError ? (openBlock(), createElementBlock("span", _hoisted_5, toDisplayString($setup.loadErrorMessage), 1)) : createCommentVNode("v-if", true),
              ...
              createBaseVNode("button", {
                type: "button",
                class: "expand-button",
                onClick: $setup.toggleChartExpansion
              }, toDisplayString($setup.chartExpanded ? "Collapse" : "Expand"), 1)
            ])
          ]),
          createBaseVNode("div", _hoisted_6, [
            createBaseVNode("div", _hoisted_7, [
              createCommentVNode(" All Jaws "),
              $setup.view === "all" ? (openBlock(), createElementBlock("div", _hoisted_8, [
                createBaseVNode("ul", _hoisted_9, [
                  (openBlock(true), createElementBlock(Fragment, null, renderList($setup.upperLeft, (num) => {
                    return openBlock(), createElementBlock("li", { key: num }, [
                      createVNode($setup["ToothCanvas"], mergeProps(toHandlers($setup.toothEventHandlers), {
                        view: $setup.view,
                        "tooth-number": num,
                        arch: "upper",
                        invertY: false,
                      ...
                      }), null, 16, ["view", "tooth-number", "condition", "pathology", "restorations", "periodontal", "previous-number", "next-number", "previous-periodontal", "next-periodontal", "show-pathology", "show-restorations", "show-periodontal", "is-selected"])
                    ]);
                  }), 128)),
                  (openBlock(true), createElementBlock(Fragment, null, renderList($setup.upperRight, (num) => {
                    return openBlock(), createElementBlock("li", { key: num }, [
                      createVNode($setup["ToothCanvas"], mergeProps(toHandlers($setup.toothEventHandlers), {
                        view: $setup.view,
                        "tooth-number": num,
                ...
                createBaseVNode("ul", _hoisted_11, [
                  (openBlock(true), createElementBlock(Fragment, null, renderList($setup.lowerLeft, (num) => {
                    return openBlock(), createElementBlock("li", { key: num }, [
                      createVNode($setup["ToothCanvas"], mergeProps(toHandlers($setup.toothEventHandlers), {
                        view: $setup.view,
                        "tooth-number": num,
                  ...
                  (openBlock(true), createElementBlock(Fragment, null, renderList($setup.lowerRight, (num) => {
                    return openBlock(), createElementBlock("li", { key: num }, [
                      createVNode($setup["ToothCanvas"], mergeProps(toHandlers($setup.toothEventHandlers), {
                        view: $setup.view,
                        "tooth-number": num,
          ...
          createBaseVNode("nav", _hoisted_21, [
            createBaseVNode("ul", null, [
              createBaseVNode("li", {
                ...
                onClick: _cache[1] || (_cache[1] = ($event) => $setup.view = "upper")
              }, [..._hoisted_23], 2),
              createBaseVNode("li", {
                id: "jaws-button",
                class: normalizeClass(["jaw-button", { "active-jaw": $setup.view === "all" }]),
                onClick: _cache[2] || (_cache[2] = ($event) => $setup.view = "all")
              }, [..._hoisted_25], 2),
              createBaseVNode("li", {
                id: "lower-jaw-button",
                class: normalizeClass(["jaw-button", { "active-jaw": $setup.view === "lower" }]),
                onClick: _cache[3] || (_cache[3] = ($event) => $setup.view = "lower")
              }, [..._hoisted_27], 2)
            ])
          ]),
        ...
        withDirectives(createBaseVNode("section", _hoisted_30, [
          createBaseVNode("div", _hoisted_31, [
            createBaseVNode("div", _hoisted_322, [
              (openBlock(), createElementBlock(Fragment, null, renderList($setup.actionOptions, (action) => {
                ...
                }, toDisplayString(action.label), 11, _hoisted_33);
              }), 64))
            ]),
            createCommentVNode(' <p class="actions-hint">Pick an action and tooth to open the work area.</p> '),
            $setup.isProcedureMode ? (openBlock(), createElementBlock("div", _hoisted_34, [
              createBaseVNode("span", null, toDisplayString($setup.selectedTeethSummary), 1),
              $setup.selectedTeeth.length ? (openBlock(), createElementBlock("button", {
                key: 0,
                type: "button",
                class: "text-link",
                onClick: $setup.clearSelectedTeeth
              }, " Clear ")) : createCommentVNode("v-if", true)
            ])) : createCommentVNode("v-if", true)
          ]),
          $setup.isOverviewMode && $setup.overviewTooth ? (openBlock(), createElementBlock("div", _hoisted_35, [
            createBaseVNode("header", _hoisted_36, [
              createBaseVNode("div", null, [
                _hoisted_37,
                createBaseVNode("p", null, "Tooth " + toDisplayString($setup.overviewTooth), 1)
              ]),
              createBaseVNode("button", {
                class: "text-link",
                onClick: $setup.resetOverviewPanel,
                type: "button"
              }, "Reset")
            ]),
            createBaseVNode("div", _hoisted_38, [
              createBaseVNode("section", null, [
                _hoisted_39,
                $setup.overviewCondition ? (openBlock(), createElementBlock("p", _hoisted_40, toDisplayString($setup.summariseCondition($setup.overviewCondition)), 1)) : (openBlock(), createElementBlock("p", _hoisted_41, "No condition recorded."))
              ]),
              createBaseVNode("section", null, [
                _hoisted_422,
                $setup.overviewRestorations.length ? (openBlock(), createElementBlock("ul", _hoisted_43, [
                  (openBlock(true), createElementBlock(Fragment, null, renderList($setup.overviewRestorations, (entry, idx) => {
                    return openBlock(), createElementBlock("li", {
                      key: `or-${idx}`
                    }, toDisplayString($setup.summariseRestoration(entry)), 1);
                  }), 128))
                ])) : (openBlock(), createElementBlock("p", _hoisted_44, "No restorations recorded."))
              ]),
              createBaseVNode("section", null, [
                _hoisted_45,
                $setup.overviewPathology.length ? (openBlock(), createElementBlock("ul", _hoisted_46, [
                  (openBlock(true), createElementBlock(Fragment, null, renderList($setup.overviewPathology, (entry, idx) => {
                    return openBlock(), createElementBlock("li", {
                      key: `op-${idx}`
                    }, toDisplayString($setup.summarisePathology(entry)), 1);
                  }), 128))
                ])) : (openBlock(), createElementBlock("p", _hoisted_47, "No pathology recorded."))
              ]),
              createBaseVNode("section", null, [
                _hoisted_48,
                $setup.overviewPeriodontal ? (openBlock(), createElementBlock("div", _hoisted_49, [
                  createBaseVNode("div", null, [
                    _hoisted_50,
                    createBaseVNode("strong", null, toDisplayString($setup.formatPeriodontalSummary($setup.overviewPeriodontal.buccal)), 1)
                  ]),
                  createBaseVNode("div", null, [
                    _hoisted_51,
                    createBaseVNode("strong", null, toDisplayString($setup.formatPeriodontalSummary($setup.overviewPeriodontal.lingual)), 1)
                  ])
                ])) : (openBlock(), createElementBlock("p", _hoisted_52, "No periodontal data."))
              ])
            ])
          ])) : createCommentVNode("v-if", true),
          createCommentVNode(' <div class="layer-card">\n        <header>\n          <h3>Layers</h3>\n          <p>Toggle visual overlays</p>\n        </header>\n      </div> '),
          !$setup.isOverviewMode ? (openBlock(), createElementBlock("div", _hoisted_53, [
                    ...
                    createBaseVNode("section", _hoisted_60, [
                      (openBlock(true), createElementBlock(Fragment, null, renderList($setup.panelSites, (site) => {
                        return openBlock(), createElementBlock("button", {
                          ...
                          createBaseVNode("span", _hoisted_62, toDisplayString($setup.siteDisplayLabel(site)), 1),
                          createBaseVNode("small", _hoisted_63, toDisplayString($setup.periodontalSiteSummary($setup.selectedTooth, site)), 1)
                        ], 10, _hoisted_61);
                      }), 128))
                    ]),
                    createBaseVNode("section", _hoisted_64, [
                      createBaseVNode("label", null, [
                        ...
                        createBaseVNode("input", {
                          ...
                          onInput: _cache[4] || (_cache[4] = ($event) => $setup.setCurrentSiteMeasurement("depth", $event.target.value)),
                      ...
                      createBaseVNode("label", null, [
                        ...
                        createBaseVNode("input", {
                          ...
                          onInput: _cache[5] || (_cache[5] = ($event) => $setup.setCurrentSiteMeasurement("margin", $event.target.value)),
                          onKeyup: withKeys($setup.confirmPeriodontalEntry, ["enter"])
                        }, null, 40, _hoisted_68)
                      ])
                    ]),
                    ...
                    createBaseVNode("section", _hoisted_71, [
                      createBaseVNode("label", _hoisted_72, [
                        withDirectives(createBaseVNode("input", {
                          type: "checkbox",
                          "onUpdate:modelValue": _cache[6] || (_cache[6] = ($event) => $setup.periodontalAutoAdvance = $event)
                        }, null, 512), [
                      ...
                      createBaseVNode("div", _hoisted_74, [
                        ...
                        }, toDisplayString($setup.savingPeriodontal ? "Saving\u2026" : "Save Test"), 9, _hoisted_75)
                      ]),
                      $setup.periodontalSaveMessage ? (openBlock(), createElementBlock("p", _hoisted_76, toDisplayString($setup.periodontalSaveMessage), 1)) : createCommentVNode("v-if", true)
                    ])
            ...
            createVNode(Transition, { name: "panel-slide" }, {
              default: withCtx(() => [
                $setup.isConditionMode && $setup.conditionPanelVisible && $setup.conditionTooth ? (openBlock(), createElementBlock("aside", _hoisted_77, [
                  createBaseVNode("header", _hoisted_78, [
                    createBaseVNode("div", null, [
                      _hoisted_79,
                      createBaseVNode("p", _hoisted_80, "Tooth " + toDisplayString($setup.conditionTooth), 1)
                    ]),
                    createBaseVNode("button", {
                      type: "button",
                      class: "panel-close",
                  ...
                  createBaseVNode("section", _hoisted_81, [
                    $setup.activeConditionEntry ? (openBlock(), createElementBlock("div", _hoisted_82, [
                      _hoisted_83,
                    ...
                    createBaseVNode("div", _hoisted_84, [
                      ...
                      withDirectives(createBaseVNode("select", {
                        id: "condition-state",
                        "onUpdate:modelValue": _cache[7] || (_cache[7] = ($event) => $setup.conditionForm.state = $event)
                      }, [
                        _hoisted_86,
                    ...
                    createBaseVNode("div", _hoisted_88, [
                      ...
                      withDirectives(createBaseVNode("select", {
                        id: "eruption-status",
                        "onUpdate:modelValue": _cache[8] || (_cache[8] = ($event) => $setup.conditionForm.eruption_status = $event)
                      }, [
                        _hoisted_90,
                    ...
                    createBaseVNode("div", _hoisted_92, [
                      ...
                      withDirectives(createBaseVNode("input", {
                        ...
                        max: "9",
                        step: "1",
                        "onUpdate:modelValue": _cache[9] || (_cache[9] = ($event) => $setup.conditionForm.mobility = $event),
                    ...
                    createBaseVNode("div", _hoisted_94, [
                      ...
                      withDirectives(createBaseVNode("input", {
                        ...
                        max: "9",
                        step: "1",
                        "onUpdate:modelValue": _cache[10] || (_cache[10] = ($event) => $setup.conditionForm.furcation = $event),
                        placeholder: "Enter a number"
                      }, null, 512), [
                    ...
                    createBaseVNode("div", _hoisted_96, [
                      ...
                      withDirectives(createBaseVNode("textarea", {
                        id: "condition-notes",
                        rows: "3",
                        "onUpdate:modelValue": _cache[11] || (_cache[11] = ($event) => $setup.conditionForm.condition_notes = $event),
                        placeholder: "Add notes (optional)"
                      }, null, 512), [
                        [vModelText, $setup.conditionForm.condition_notes]
                      ])
                    ]),
                    $setup.conditionError ? (openBlock(), createElementBlock("p", _hoisted_98, toDisplayString($setup.conditionError), 1)) : createCommentVNode("v-if", true)
                  ]),
                  createBaseVNode("footer", _hoisted_99, [
                    createBaseVNode("button", {
                      type: "button",
                      class: "secondary",
                      onClick: $setup.closeConditionPanel
                    ...
                    }, toDisplayString($setup.savingCondition ? "Saving\u2026" : "Save Condition"), 9, _hoisted_100)
                  ])
                ])) : createCommentVNode("v-if", true)
              ]),
              _: 1
            ...
            createVNode(Transition, { name: "panel-slide" }, {
              default: withCtx(() => [
                $setup.isPathologyMode && $setup.pathologyPanelVisible && $setup.pathologyTooth ? (openBlock(), createElementBlock("aside", _hoisted_101, [
                  createBaseVNode("header", _hoisted_102, [
                    createBaseVNode("div", null, [
                      _hoisted_103,
                      createBaseVNode("p", _hoisted_104, "Tooth " + toDisplayString($setup.pathologyTooth), 1)
                    ]),
                    createBaseVNode("button", {
                      type: "button",
                      class: "panel-close",
                  ...
                  createBaseVNode("section", _hoisted_105, [
                    $setup.activePathologyEntries.length ? (openBlock(), createElementBlock("div", _hoisted_106, [
                      _hoisted_107,
                      createBaseVNode("ul", null, [
                        (openBlock(true), createElementBlock(Fragment, null, renderList($setup.activePathologyEntries, (entry, idx) => {
                          return openBlock(), createElementBlock("li", {
                            ...
                            createBaseVNode("span", _hoisted_108, [
                              createBaseVNode("button", {
                                type: "button",
                                class: "text-button",
                    ...
                    createBaseVNode("div", _hoisted_111, [
                      ...
                      withDirectives(createBaseVNode("select", {
                        id: "pathology-type",
                        "onUpdate:modelValue": _cache[12] || (_cache[12] = ($event) => $setup.pathologyForm.pathology_type = $event)
                      }, [
                        _hoisted_113,
                    ...
                    $setup.pathologyForm.pathology_type === "Decay" ? (openBlock(), createElementBlock("div", _hoisted_115, [
                      createBaseVNode("div", _hoisted_116, [
                        _hoisted_117,
                        withDirectives(createBaseVNode("select", {
                          id: "decay-stage",
                          "onUpdate:modelValue": _cache[13] || (_cache[13] = ($event) => $setup.pathologyForm.decay_stage = $event)
                        }, [
                          _hoisted_118,
                      ...
                      createBaseVNode("div", _hoisted_120, [
                        ...
                        withDirectives(createBaseVNode("select", {
                          id: "decay-cavitation",
                          "onUpdate:modelValue": _cache[14] || (_cache[14] = ($event) => $setup.pathologyForm.cavitation = $event)
                        }, [
                          _hoisted_1222,
                          (openBlock(), createElementBlock(Fragment, null, renderList($setup.CAVITATION_OPTIONS, (option) => {
                            return createBaseVNode("option", {
                              ...
                              value: option
                            }, toDisplayString(option), 9, _hoisted_123);
                          }), 64))
                        ], 512), [
                          [vModelSelect, $setup.pathologyForm.cavitation]
                        ])
                      ]),
                      $setup.pathologyForm.cavitation === "Cavitation" ? (openBlock(), createElementBlock("div", _hoisted_124, [
                        _hoisted_125,
                        withDirectives(createBaseVNode("select", {
                          id: "decay-pulp",
                          "onUpdate:modelValue": _cache[15] || (_cache[15] = ($event) => $setup.pathologyForm.pulp = $event)
                        }, [
                          _hoisted_126,
                          (openBlock(), createElementBlock(Fragment, null, renderList($setup.PULP_OPTIONS, (option) => {
                            return createBaseVNode("option", {
                              key: `pulp-${option}`,
                              value: option
                            }, toDisplayString(option), 9, _hoisted_127);
                          }), 64))
                        ], 512), [
                      ...
                      createBaseVNode("div", _hoisted_128, [
                        ...
                        withDirectives(createBaseVNode("select", {
                          id: "decay-level",
                          "onUpdate:modelValue": _cache[16] || (_cache[16] = ($event) => $setup.pathologyForm.decay_level = $event)
                        }, [
                          _hoisted_130,
                          (openBlock(), createElementBlock(Fragment, null, renderList($setup.DECAY_LEVEL_OPTIONS, (option) => {
                            return createBaseVNode("option", {
                              ...
                              value: option
                            }, toDisplayString(option), 9, _hoisted_131);
                          }), 64))
                        ], 512), [
                      ...
                      createBaseVNode("div", _hoisted_132, [
                        _hoisted_133,
                        $setup.surfaceOptions.length ? (openBlock(), createElementBlock("div", _hoisted_134, [
                          (openBlock(true), createElementBlock(Fragment, null, renderList($setup.surfaceOptions, (surface) => {
                            return openBlock(), createElementBlock("label", {
                              ...
                              withDirectives(createBaseVNode("input", {
                                type: "checkbox",
                                value: surface.value,
                                "onUpdate:modelValue": _cache[17] || (_cache[17] = ($event) => $setup.pathologyForm.decay_surfaces = $event)
                              }, null, 8, _hoisted_135), [
                        ...
                        ])) : $setup.surfacesLoading ? (openBlock(), createElementBlock("p", _hoisted_136, "Loading surfaces\u2026")) : (openBlock(), createElementBlock("p", _hoisted_137, "Define tooth surfaces to pick specific zones."))
                      ])
                    ])) : createCommentVNode("v-if", true),
                    $setup.pathologyForm.pathology_type === "Fracture" ? (openBlock(), createElementBlock("div", _hoisted_138, [
                      createBaseVNode("div", _hoisted_139, [
                        _hoisted_140,
                        withDirectives(createBaseVNode("select", {
                          id: "fracture-type",
                          "onUpdate:modelValue": _cache[18] || (_cache[18] = ($event) => $setup.pathologyForm.fracture_type = $event)
                        }, [
                          _hoisted_141,
                          (openBlock(), createElementBlock(Fragment, null, renderList($setup.FRACTURE_TYPE_OPTIONS, (option) => {
                            return createBaseVNode("option", {
                              ...
                              value: option
                            }, toDisplayString(option), 9, _hoisted_142);
                          }), 64))
                        ], 512), [
                      ...
                      createBaseVNode("div", _hoisted_143, [
                        ...
                        withDirectives(createBaseVNode("select", {
                          ...
                          "onUpdate:modelValue": _cache[19] || (_cache[19] = ($event) => $setup.pathologyForm.fracture_position = $event)
                        }, [
                          _hoisted_145,
                          (openBlock(), createElementBlock(Fragment, null, renderList($setup.FRACTURE_POSITION_OPTIONS, (option) => {
                            return createBaseVNode("option", {
                              ...
                              value: option
                            }, toDisplayString(option), 9, _hoisted_146);
                          }), 64))
                        ], 512), [
                    ...
                    $setup.pathologyForm.pathology_type === "Tooth Wear" ? (openBlock(), createElementBlock("div", _hoisted_147, [
                      createBaseVNode("div", _hoisted_148, [
                        _hoisted_149,
                        withDirectives(createBaseVNode("select", {
                          id: "wear-type",
                          "onUpdate:modelValue": _cache[20] || (_cache[20] = ($event) => $setup.pathologyForm.wear_type = $event)
                        }, [
                          _hoisted_150,
                          (openBlock(), createElementBlock(Fragment, null, renderList($setup.WEAR_TYPE_OPTIONS, (option) => {
                            return createBaseVNode("option", {
                              ...
                              value: option
                            }, toDisplayString(option), 9, _hoisted_151);
                          }), 64))
                        ], 512), [
                      ...
                      createBaseVNode("div", _hoisted_152, [
                        _hoisted_153,
                        $setup.surfaceOptions.length ? (openBlock(), createElementBlock("div", _hoisted_154, [
                          (openBlock(true), createElementBlock(Fragment, null, renderList($setup.surfaceOptions, (surface) => {
                            return openBlock(), createElementBlock("label", {
                              ...
                              withDirectives(createBaseVNode("input", {
                                type: "checkbox",
                                value: surface.value,
                                "onUpdate:modelValue": _cache[21] || (_cache[21] = ($event) => $setup.pathologyForm.wear_surfaces = $event)
                              }, null, 8, _hoisted_155), [
                        ...
                        ])) : $setup.surfacesLoading ? (openBlock(), createElementBlock("p", _hoisted_156, "Loading surfaces\u2026")) : (openBlock(), createElementBlock("p", _hoisted_157, "Define tooth surfaces to pick specific zones."))
                      ])
                    ])) : createCommentVNode("v-if", true),
                    $setup.pathologyForm.pathology_type === "Discoloration" ? (openBlock(), createElementBlock("div", _hoisted_158, [
                      createBaseVNode("div", _hoisted_159, [
                        _hoisted_160,
                        withDirectives(createBaseVNode("select", {
                          id: "discoloration-type",
                          "onUpdate:modelValue": _cache[22] || (_cache[22] = ($event) => $setup.pathologyForm.discoloration_type = $event)
                        }, [
                          _hoisted_161,
                          (openBlock(), createElementBlock(Fragment, null, renderList($setup.DISCOLORATION_TYPE_OPTIONS, (option) => {
                            return createBaseVNode("option", {
                              ...
                              value: option
                            }, toDisplayString(option), 9, _hoisted_162);
                          }), 64))
                        ], 512), [
                    ...
                    createBaseVNode("div", _hoisted_163, [
                      ...
                      withDirectives(createBaseVNode("textarea", {
                        id: "pathology-notes",
                        rows: "3",
                        "onUpdate:modelValue": _cache[23] || (_cache[23] = ($event) => $setup.pathologyForm.notes = $event),
                        placeholder: "Add notes (optional)"
                      }, null, 512), [
                        [vModelText, $setup.pathologyForm.notes]
                      ])
                    ]),
                    $setup.pathologyError ? (openBlock(), createElementBlock("p", _hoisted_165, toDisplayString($setup.pathologyError), 1)) : createCommentVNode("v-if", true)
                  ]),
                  createBaseVNode("footer", _hoisted_166, [
                    createBaseVNode("button", {
                      type: "button",
                      class: "secondary",
                      onClick: $setup.closePathologyPanel
                    ...
                    }, toDisplayString($setup.savingPathology ? "Saving\u2026" : "Save Pathology"), 9, _hoisted_167)
                  ])
                ])) : createCommentVNode("v-if", true)
              ]),
              _: 1
            ...
            createVNode(Transition, { name: "panel-slide" }, {
              default: withCtx(() => [
                $setup.isRestorationMode && $setup.restorationPanelVisible && $setup.restorationTooth ? (openBlock(), createElementBlock("aside", _hoisted_168, [
                  createBaseVNode("header", _hoisted_169, [
                    createBaseVNode("div", null, [
                      _hoisted_170,
                      createBaseVNode("p", _hoisted_171, "Tooth " + toDisplayString($setup.restorationTooth), 1)
                    ]),
                    createBaseVNode("button", {
                      type: "button",
                      class: "panel-close",
                  ...
                  createBaseVNode("section", _hoisted_172, [
                    $setup.activeRestorationEntries.length ? (openBlock(), createElementBlock("div", _hoisted_173, [
                      _hoisted_174,
                      createBaseVNode("ul", null, [
                    ...
                    createBaseVNode("div", _hoisted_178, [
                      ...
                      withDirectives(createBaseVNode("select", {
                        id: "restoration-type",
                        "onUpdate:modelValue": _cache[24] || (_cache[24] = ($event) => $setup.restorationForm.restoration_type = $event)
                      }, [
                        _hoisted_180,
                        (openBlock(), createElementBlock(Fragment, null, renderList($setup.RESTORATION_TYPE_OPTIONS, (option) => {
                          return createBaseVNode("option", {
                            ...
                            value: option
                          }, toDisplayString(option), 9, _hoisted_181);
                        }), 64))
                      ], 512), [
                    ...
                    $setup.showRestorationDetailFields ? (openBlock(), createElementBlock(Fragment, { key: 1 }, [
                      createBaseVNode("div", _hoisted_182, [
                        ...
                        withDirectives(createBaseVNode("select", {
                          ...
                          "onUpdate:modelValue": _cache[25] || (_cache[25] = ($event) => $setup.restorationForm.material = $event)
                        }, [
                          _hoisted_184,
                          (openBlock(), createElementBlock(Fragment, null, renderList($setup.RESTORATION_MATERIAL_OPTIONS, (option) => {
                            return createBaseVNode("option", {
                              ...
                              value: option
                            }, toDisplayString(option), 9, _hoisted_185);
                          }), 64))
                        ], 512), [
                      ...
                      createBaseVNode("div", _hoisted_186, [
                        _hoisted_187,
                        $setup.surfaceOptions.length ? (openBlock(), createElementBlock("div", _hoisted_188, [
                          (openBlock(true), createElementBlock(Fragment, null, renderList($setup.surfaceOptions, (surface) => {
                            return openBlock(), createElementBlock("label", {
                              ...
                              withDirectives(createBaseVNode("input", {
                                type: "checkbox",
                                value: surface.value,
                                "onUpdate:modelValue": _cache[26] || (_cache[26] = ($event) => $setup.restorationForm.surfaces = $event)
                              }, null, 8, _hoisted_189), [
                        ...
                        ])) : $setup.surfacesLoading ? (openBlock(), createElementBlock("p", _hoisted_190, "Loading surfaces\u2026")) : (openBlock(), createElementBlock("p", _hoisted_191, "Define tooth surfaces to pick specific zones."))
                      ]),
                      createBaseVNode("div", _hoisted_192, [
                        _hoisted_193,
                        withDirectives(createBaseVNode("select", {
                          id: "restoration-quality",
                          "onUpdate:modelValue": _cache[27] || (_cache[27] = ($event) => $setup.restorationForm.quality = $event)
                        }, [
                          _hoisted_194,
                          (openBlock(), createElementBlock(Fragment, null, renderList($setup.RESTORATION_QUALITY_OPTIONS, (option) => {
                            return createBaseVNode("option", {
                              ...
                              value: option
                            }, toDisplayString(option), 9, _hoisted_195);
                          }), 64))
                        ], 512), [
                      ...
                      createBaseVNode("div", _hoisted_196, [
                        ...
                        withDirectives(createBaseVNode("select", {
                          ...
                          "onUpdate:modelValue": _cache[28] || (_cache[28] = ($event) => $setup.restorationForm.detail = $event)
                        }, [
                          _hoisted_198,
                          (openBlock(), createElementBlock(Fragment, null, renderList($setup.RESTORATION_DETAIL_OPTIONS, (option) => {
                            return createBaseVNode("option", {
                              ...
                              value: option
                            }, toDisplayString(option), 9, _hoisted_199);
                          }), 64))
                        ], 512), [
                      ...
                      createBaseVNode("div", _hoisted_200, [
                        ...
                        withDirectives(createBaseVNode("select", {
                          id: "crown-material",
                          "onUpdate:modelValue": _cache[29] || (_cache[29] = ($event) => $setup.restorationForm.crown_material = $event)
                        }, [
                          _hoisted_202,
                          (openBlock(), createElementBlock(Fragment, null, renderList($setup.CROWN_MATERIAL_OPTIONS, (option) => {
                            return createBaseVNode("option", {
                              ...
                              value: option
                            }, toDisplayString(option), 9, _hoisted_203);
                          }), 64))
                        ], 512), [
                      ...
                      createBaseVNode("div", _hoisted_204, [
                        ...
                        withDirectives(createBaseVNode("select", {
                          id: "crown-type",
                          "onUpdate:modelValue": _cache[30] || (_cache[30] = ($event) => $setup.restorationForm.crown_type = $event)
                        }, [
                          _hoisted_206,
                          (openBlock(), createElementBlock(Fragment, null, renderList($setup.CROWN_TYPE_OPTIONS, (option) => {
                            return createBaseVNode("option", {
                              ...
                              value: option
                            }, toDisplayString(option), 9, _hoisted_207);
                          }), 64))
                        ], 512), [
                      ...
                      createBaseVNode("div", _hoisted_208, [
                        ...
                        withDirectives(createBaseVNode("select", {
                          id: "crown-base",
                          "onUpdate:modelValue": _cache[31] || (_cache[31] = ($event) => $setup.restorationForm.crown_base = $event)
                        }, [
                          _hoisted_210,
                          (openBlock(), createElementBlock(Fragment, null, renderList($setup.CROWN_BASE_OPTIONS, (option) => {
                            return createBaseVNode("option", {
                              ...
                              value: option
                            }, toDisplayString(option), 9, _hoisted_211);
                          }), 64))
                        ], 512), [
                          [vModelSelect, $setup.restorationForm.crown_base]
                        ])
                      ]),
                      $setup.showImplantFields ? (openBlock(), createElementBlock("div", _hoisted_212, [
                        _hoisted_213,
                        withDirectives(createBaseVNode("select", {
                          id: "crown-implant-type",
                          "onUpdate:modelValue": _cache[32] || (_cache[32] = ($event) => $setup.restorationForm.crown_implant_type = $event)
                        }, [
                          _hoisted_214,
                          (openBlock(), createElementBlock(Fragment, null, renderList($setup.CROWN_IMPLANT_TYPE_OPTIONS, (option) => {
                            return createBaseVNode("option", {
                              ...
                              value: option
                            }, toDisplayString(option), 9, _hoisted_215);
                          }), 64))
                        ], 512), [
                    ...
                    $setup.restorationError ? (openBlock(), createElementBlock("p", _hoisted_216, toDisplayString($setup.restorationError), 1)) : createCommentVNode("v-if", true)
                  ]),
                  createBaseVNode("footer", _hoisted_217, [
                    createBaseVNode("button", {
                      type: "button",
                      class: "secondary",
                      onClick: $setup.closeRestorationPanel
                    ...
                    }, toDisplayString($setup.savingRestoration ? "Saving\u2026" : "Save Restoration"), 9, _hoisted_218)
                  ])
                ])) : createCommentVNode("v-if", true)
              ]),
              _: 1
            ...
            createVNode(Transition, { name: "panel-slide" }, {
              default: withCtx(() => [
                $setup.isProcedureMode && $setup.procedurePanelVisible ? (openBlock(), createElementBlock("aside", _hoisted_219, [
                  createBaseVNode("header", _hoisted_220, [
                    createBaseVNode("div", null, [
                      _hoisted_221,
                      $setup.selectedProcedureTemplate ? (openBlock(), createElementBlock("p", _hoisted_2222, toDisplayString($setup.selectedProcedureTemplate.template), 1)) : (openBlock(), createElementBlock("p", _hoisted_223, "Select a template to begin"))
                    ]),
                    createBaseVNode("button", {
                      type: "button",
                      class: "panel-close",
                      onClick: $setup.closeProcedurePanel,
                      "aria-label": "Close procedure panel"
                    }, " \xD7 ")
                  ]),
                  createBaseVNode("section", _hoisted_224, [
                    createBaseVNode("div", _hoisted_225, [
                      createBaseVNode("strong", null, toDisplayString($setup.selectedTeeth.length), 1),
                      _hoisted_226,
                      createBaseVNode("button", {
                        type: "button",
                        class: "text-link",
                    ...
                    createBaseVNode("div", _hoisted_227, [
                      ...
                      withDirectives(createBaseVNode("input", {
                        id: "procedure-search",
                        type: "search",
                        "onUpdate:modelValue": _cache[33] || (_cache[33] = ($event) => $setup.procedureTemplateSearch = $event),
                        placeholder: "Search template name",
                        onFocus: _cache[34] || (_cache[34] = ($event) => $setup.ensureProcedureTemplates())
                      }, null, 544), [
                        [vModelText, $setup.procedureTemplateSearch]
                      ])
                    ]),
                    createBaseVNode("div", _hoisted_229, [
                      (openBlock(true), createElementBlock(Fragment, null, renderList($setup.filteredProcedureTemplates, (template) => {
                        return openBlock(), createElementBlock("button", {
                          key: template.name,
                          type: "button",
                          class: normalizeClass({ selected: $setup.selectedProcedureTemplate && $setup.selectedProcedureTemplate.name === template.name }),
                          onClick: ($event) => $setup.selectProcedureTemplate(template)
                        }, [
                          createBaseVNode("strong", null, toDisplayString(template.template), 1),
                          createBaseVNode("small", null, toDisplayString(template.description || template.item_group), 1),
                          template.standard_price !== null && template.standard_price !== void 0 ? (openBlock(), createElementBlock("em", _hoisted_231, " Standard Price: " + toDisplayString($setup.formatCurrency(template.standard_price, template.standard_price_currency)), 1)) : template.rate ? (openBlock(), createElementBlock("em", _hoisted_232, " Rate: " + toDisplayString($setup.formatCurrency(template.rate)), 1)) : createCommentVNode("v-if", true)
                        ], 10, _hoisted_230);
                      }), 128)),
                      !$setup.procedureTemplatesLoading && !$setup.filteredProcedureTemplates.length ? (openBlock(), createElementBlock("p", _hoisted_233, " No templates found. ")) : createCommentVNode("v-if", true),
                      $setup.procedureTemplatesLoading ? (openBlock(), createElementBlock("p", _hoisted_234, "Loading templates\u2026")) : createCommentVNode("v-if", true)
                    ]),
                    $setup.selectedProcedureTemplate ? (openBlock(), createElementBlock("div", _hoisted_235, [
                      createBaseVNode("div", _hoisted_236, [
                        _hoisted_237,
                        withDirectives(createBaseVNode("select", {
                          id: "procedure-type",
                          "onUpdate:modelValue": _cache[35] || (_cache[35] = ($event) => $setup.procedureForm.restoration_type = $event)
                        }, [
                          _hoisted_238,
                          (openBlock(), createElementBlock(Fragment, null, renderList($setup.RESTORATION_TYPE_OPTIONS, (option) => {
                            return createBaseVNode("option", {
                              ...
                              value: option
                            }, toDisplayString(option), 9, _hoisted_239);
                          }), 64))
                        ], 512), [
                          [vModelSelect, $setup.procedureForm.restoration_type]
                        ])
                      ]),
                      createBaseVNode("div", _hoisted_240, [
                        ...
                        withDirectives(createBaseVNode("select", {
                          ...
                          "onUpdate:modelValue": _cache[36] || (_cache[36] = ($event) => $setup.procedureForm.material = $event)
                        }, [
                          _hoisted_242,
                          (openBlock(), createElementBlock(Fragment, null, renderList($setup.RESTORATION_MATERIAL_OPTIONS, (option) => {
                            return createBaseVNode("option", {
                              ...
                              value: option
                            }, toDisplayString(option), 9, _hoisted_243);
                          }), 64))
                        ], 512), [
                          [vModelSelect, $setup.procedureForm.material]
                        ])
                      ]),
                      createBaseVNode("div", _hoisted_244, [
                        _hoisted_245,
                        $setup.surfaceOptions.length ? (openBlock(), createElementBlock("div", _hoisted_246, [
                          (openBlock(true), createElementBlock(Fragment, null, renderList($setup.surfaceOptions, (surface) => {
                            return openBlock(), createElementBlock("label", {
                              key: `procedure-surface-${surface.value}`,
                              class: normalizeClass({ selected: $setup.procedureForm.surfaces.includes(surface.value) })
                            }, [
                              withDirectives(createBaseVNode("input", {
                                type: "checkbox",
                                value: surface.value,
                                "onUpdate:modelValue": _cache[37] || (_cache[37] = ($event) => $setup.procedureForm.surfaces = $event)
                              }, null, 8, _hoisted_247), [
                                [vModelCheckbox, $setup.procedureForm.surfaces]
                              ]),
                              createBaseVNode("span", null, toDisplayString(surface.label), 1)
                            ], 2);
                          }), 128))
                        ])) : (openBlock(), createElementBlock("p", _hoisted_248, "Define tooth surfaces to pick specific zones."))
                      ]),
                      createBaseVNode("div", _hoisted_249, [
                        _hoisted_250,
                        createBaseVNode("div", _hoisted_251, null, 512)
                      ]),
                      createBaseVNode("div", _hoisted_252, [
                        ...
                        withDirectives(createBaseVNode("textarea", {
                          id: "procedure-notes",
                          rows: "3",
                          "onUpdate:modelValue": _cache[38] || (_cache[38] = ($event) => $setup.procedureForm.notes = $event),
                          placeholder: "Describe the procedure details"
                        }, null, 512), [
                          [vModelText, $setup.procedureForm.notes]
                        ])
                      ])
                    ])) : createCommentVNode("v-if", true),
                    $setup.procedureError ? (openBlock(), createElementBlock("p", _hoisted_254, toDisplayString($setup.procedureError), 1)) : createCommentVNode("v-if", true)
                  ]),
                  createBaseVNode("footer", _hoisted_255, [
                    createBaseVNode("button", {
                      type: "button",
                      class: "secondary",
                      onClick: $setup.closeProcedurePanel
                    ...
                    createBaseVNode("button", {
                      type: "button",
                      class: "primary",
                      disabled: !$setup.canSubmitProcedure || $setup.savingProcedure,
                      onClick: $setup.submitProcedure
                    }, toDisplayString($setup.savingProcedure ? "Saving\u2026" : "Finish Procedure"), 9, _hoisted_256)
                  ])
                ])) : createCommentVNode("v-if", true)
              ]),
              _: 1
            }),
            !$setup.hasActivePanel ? (openBlock(), createElementBlock("p", _hoisted_257, " Select a tooth to review or update the current action. ")) : createCommentVNode("v-if", true)
          ])) : createCommentVNode("v-if", true)
        ], 512), [
          [vShow, !$setup.chartExpanded]
        ])
      ], 2),
      createBaseVNode("section", _hoisted_258, [
        createBaseVNode("header", null, [
          _hoisted_259,
          createBaseVNode("span", _hoisted_260, toDisplayString($setup.visitHistory.length), 1)
        ]),
        !$setup.visitHistory.length ? (openBlock(), createElementBlock("div", _hoisted_261, " Visits will appear here after encounters or procedures are recorded. ")) : createCommentVNode("v-if", true),
        createBaseVNode("div", _hoisted_262, [
          createBaseVNode("table", _hoisted_263, [
            _hoisted_264,
            createBaseVNode("tbody", null, [
              (openBlock(true), createElementBlock(Fragment, null, renderList($setup.groupedVisitHistory, (group) => {
                return openBlock(), createElementBlock(Fragment, {
                  key: group.key
                }, [
                  createBaseVNode("tr", {
                    class: normalizeClass($setup.groupRowClass(group)),
                    onClick: ($event) => $setup.toggleAppointmentGroup(group.key)
                  }, [
                    createBaseVNode("td", _hoisted_266, [
                      createBaseVNode("span", {
                        class: normalizeClass(["chevron", { open: !$setup.isAppointmentCollapsed(group.key) }])
                      }, "\u25B8", 2),
                      createBaseVNode("div", _hoisted_267, [
                        createBaseVNode("span", _hoisted_268, toDisplayString(group.appointment ? `Appointment ${group.appointment}` : "No Appointment"), 1)
                      ])
                    ]),
                    createBaseVNode("td", null, toDisplayString(group.appointment ? $setup.formatVisitTimestamp(group.summary.date, group.summary.time) : ""), 1),
                    createBaseVNode("td", null, toDisplayString(group.appointment ? group.summary.provider || "\u2014" : ""), 1),
                    createBaseVNode("td", null, toDisplayString(group.appointment ? group.summary.status || "\u2014" : ""), 1),
                    createBaseVNode("td", null, [
                      createBaseVNode("div", _hoisted_269, [
                        createBaseVNode("span", _hoisted_270, toDisplayString(group.appointment ? $setup.formatCurrency(group.summary.billing_amount, group.summary.currency) || "\u2014" : ""), 1),
                        group.summary.paid_amount ? (openBlock(), createElementBlock("small", _hoisted_271, " Paid " + toDisplayString(group.appointment ? $setup.formatCurrency(group.summary.paid_amount, group.summary.currency) : ""), 1)) : group.summary.outstanding_amount ? (openBlock(), createElementBlock("small", _hoisted_272, " Due " + toDisplayString(group.appointment ? $setup.formatCurrency(group.summary.outstanding_amount, group.summary.currency) : ""), 1)) : createCommentVNode("v-if", true)
                      ])
                    ]),
                    _hoisted_273
                  ], 10, _hoisted_265),
                  !$setup.isAppointmentCollapsed(group.key) ? (openBlock(true), createElementBlock(Fragment, { key: 0 }, renderList(group.visits, (visit) => {
                    var _a, _b, _c, _d, _e;
                    return openBlock(), createElementBlock(Fragment, {
                      key: visit.id
                    }, [
                      createBaseVNode("tr", {
                        class: normalizeClass($setup.visitRowClass(visit)),
                        onClick: ($event) => $setup.toggleVisitActions(visit),
                        onContextmenu: withModifiers(($event) => $setup.toggleVisitActions(visit), ["prevent"])
                      }, [
                        createBaseVNode("td", null, [
                          createBaseVNode("div", _hoisted_275, [
                            createBaseVNode("strong", null, toDisplayString(visit.type_label), 1),
                            createBaseVNode("span", _hoisted_276, toDisplayString(visit.purpose || visit.title), 1)
                          ])
                        ]),
                        createBaseVNode("td", null, toDisplayString($setup.formatVisitTimestamp(visit.date, visit.time)), 1),
                        createBaseVNode("td", null, toDisplayString(visit.practitioner_name || visit.practitioner || "\u2014"), 1),
                        createBaseVNode("td", null, toDisplayString(visit.status || "\u2014"), 1),
                        _hoisted_277,
                        createBaseVNode("td", _hoisted_278, [
                          $setup.activeVisitActionId === visit.id ? (openBlock(), createElementBlock("div", {
                            key: 0,
                            class: "visit-menu",
                            onMouseleave: _cache[39] || (_cache[39] = ($event) => $setup.activeVisitActionId = null),
                            onClick: _cache[40] || (_cache[40] = withModifiers(() => {
                            }, ["stop"]))
                          }, [
                            createBaseVNode("button", {
                              type: "button",
                              onClick: ($event) => $setup.openVisitDocument(visit)
                            }, "Open Record", 8, _hoisted_279),
                            createBaseVNode("button", {
                              type: "button",
                              disabled: $setup.completingVisitId === visit.id || visit.status === "Completed",
                              onClick: ($event) => $setup.markVisitComplete(visit)
                            }, toDisplayString($setup.completingVisitId === visit.id ? "Marking\u2026" : "Set Complete"), 9, _hoisted_280),
                            createBaseVNode("button", {
                              type: "button",
                              onClick: ($event) => $setup.toggleVisitDetails(visit.id)
                            }, toDisplayString($setup.isVisitExpanded(visit.id) ? "Hide Chart Changes" : "View Chart Changes"), 9, _hoisted_281),
                            createBaseVNode("button", {
                              type: "button",
                              class: "danger",
                              disabled: $setup.deletingVisitId === visit.id,
                              onClick: ($event) => $setup.deleteVisit(visit)
                            }, toDisplayString($setup.deletingVisitId === visit.id ? "Deleting\u2026" : "Delete Visit"), 9, _hoisted_282)
                          ], 32)) : createCommentVNode("v-if", true)
                        ])
                      ], 42, _hoisted_274),
                      $setup.isVisitExpanded(visit.id) ? (openBlock(), createElementBlock("tr", _hoisted_283, [
                        createBaseVNode("td", _hoisted_284, [
                          createBaseVNode("div", _hoisted_285, [
                            ((_b = (_a = visit.changes) == null ? void 0 : _a.restorations) == null ? void 0 : _b.length) ? (openBlock(), createElementBlock(Fragment, { key: 0 }, [
                              _hoisted_286,
                              createBaseVNode("ul", null, [
                                (openBlock(true), createElementBlock(Fragment, null, renderList(visit.changes.restorations, (entry, idx) => {
                                  return openBlock(), createElementBlock("li", {
                                    key: `rest-${visit.id}-${idx}`
                                  }, toDisplayString($setup.summariseRestoration(entry)), 1);
                                }), 128))
                              ])
                            ], 64)) : createCommentVNode("v-if", true),
                            ((_d = (_c = visit.changes) == null ? void 0 : _c.pathology) == null ? void 0 : _d.length) ? (openBlock(), createElementBlock(Fragment, { key: 1 }, [
                              _hoisted_287,
                              createBaseVNode("ul", null, [
                                (openBlock(true), createElementBlock(Fragment, null, renderList(visit.changes.pathology, (entry, idx) => {
                                  return openBlock(), createElementBlock("li", {
                                    key: `pathology-${visit.id}-${idx}`
                                  }, toDisplayString($setup.summarisePathology(entry)), 1);
                                }), 128))
                              ])
                            ], 64)) : createCommentVNode("v-if", true),
                            ((_e = visit.invoices) == null ? void 0 : _e.length) ? (openBlock(), createElementBlock("div", _hoisted_288, [
                              (openBlock(true), createElementBlock(Fragment, null, renderList(visit.invoices, (invoice) => {
                                return openBlock(), createElementBlock("button", {
                                  key: `${visit.id}-${invoice.name}`,
                                  type: "button",
                                  class: "chip",
                                  onClick: ($event) => $setup.openInvoice(invoice)
                                }, toDisplayString(invoice.name) + " \xB7 " + toDisplayString($setup.formatCurrency(invoice.amount, visit.currency)), 9, _hoisted_289);
                              }), 128))
                            ])) : createCommentVNode("v-if", true)
                          ])
                        ])
      ...
      createVNode(Transition, { name: "legend-fade" }, {
        default: withCtx(() => [
          $setup.showLegend ? (openBlock(), createElementBlock("div", {
            ...
            onClick: _cache[42] || (_cache[42] = withModifiers(($event) => $setup.showLegend = false, ["self"]))
          }, [
            createBaseVNode("div", _hoisted_290, [
              createBaseVNode("header", null, [
                ...
                createBaseVNode("button", {
                  ...
                  onClick: _cache[41] || (_cache[41] = ($event) => $setup.showLegend = false)
                }, "\xD7")
              ]),
              _hoisted_292
            ])
...
* @license MIT
**/
//# sourceMappingURL=chart.bundle.CBPXB2D6.js.map
